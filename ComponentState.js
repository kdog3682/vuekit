...
class ComponentState {
    ...
    handleInlineFunction(node, { name, params, fn }) {
        const base = must(attributionalFunctionRef, name)
        ...
        const functionLocation = fparse(base.functionLocation, { name, params: filteredParams })
    ...
    toString() {
        ...
        switch (key) {
            ...
            case "variable":
                const name = pascalCase(this.component.name)
    ...
    determineAdditionalPropKeys(node) {
        ...
        const ignore = Object.keys(data).concat(computedKeys)
