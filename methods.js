import {importLoader, setRootAttributes, isInputElement, getKeyAndType, elt, preventDefault, cssLoader} from "/home/kdog3682/2024-javascript/js-toolkit/browser.js"
import * as chalk from "/home/kdog3682/2024-javascript/js-toolkit/chalk.js"
// import {animate} from "/home/kdog3682/2024-javascript/js-toolkit/animate.js"
import * as animations from "/home/kdog3682/2024-javascript/js-toolkit/animate.js"
...
async function $toggle(key, o) {
    if (o == null) {
        this[key] = !this[key]
        return 
    }
    o.key = key
    const value = await asyncToggle(this, o)
    if (o.force) {
        this.$forceUpdate()
    }
    return value
...
async function $animate(el, key, ...args) {
    // console.log({key, args, el})
    try {
        return await animations[key](el, ...args)
    } catch(e) {
        console.log("ERRORRRRRRRR", e.stack)
        console.log(el, key, args)
    }
}

function $log(key, transform) {
    chalk.blue(key + ':', true)
    const value = this.$get(key, transform)
    const computed = isDefined(value) ? value : 'NULL'
    console.log(computed)
}

function $get(key, transform) {
    const get = (key) => {
        const _value = key in this.$options ? this.$options[key] : '$' + key in this ? this['$' + key] : this[key]
        return _value
    }
    const _value = get(key)
    const value = transform ? transform(_value) : _value
    return value
}

function $modular(key, value) {
    return boundModularIncrement(this, key, value)
}
...
function $info(key, source) {
    const state = source == 'parent' ? this.$parent : this
...
class VueProxy {
    ...
    // Define it on the 
    init(vue) {
        // these are the only internals allowed
        // setting is not allowed
        // accessing the vue instance is not allowed
        for (const key of this.keys) {
            const get = () => vue[key]
            Object.defineProperty(this, key, {get})
        }
        const refKeys = Object.keys(vue.$refs || {})
        for (const key of refKeys) {
            Object.defineProperty(this, key, () => this.$refs[key]
        }
...
const $proxy = new VueProxy(['uid', '$options'])
...
class Keymap {
    ...
    add(binding, func) {
        this.store[binding] = func
    }
    call(vue, e) {
        function fix(s) {
            return s.replace(/(?:Control|yypdfg)/, 'ctrl-')
            return s.replace(/(?:ctrl|yypdfg)/, 'Control')
        }
        try {
            const m = getKeyAndType(e)
            if (!m) {
                return 
            }
            const [keyName, primaryType, secondaryType] = m
            const func = this.store[fix(keyName)]
            if (!func) {
                return 
            }
            func(vue)
