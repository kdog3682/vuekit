...
const VList = {
    ...
    template: `
        <div class="v-list">
            <ul>
                <li :key="item" v-for="item in value">
...
const VImg = {
    ...
    computed: {
        src() {
            const imglib = {
                sunset: "/mnt/chromeos/MyFiles/Downloads/jordan-wozniak-xP_AGmeEa6s-unsplash.jpg"
            }
            return imglib[this.value]
        },
        containerStyle() {
            const size = this.size ? addUnit(this.size, "pt") : "50pt"
            return {
                width: size,
                height: size,
...
const VHtml = {
    ...
    render(h) {
        ...
        return h("div", { class: "v-html", domProps: { innerHTML } })
    },
    props: ["value"]
}
