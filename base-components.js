...
const VDate = {
    ...
    render(h) {
        const dt = datetime()
        return renderWrapper(this, "div", dt.toString())
    },
    props: ["value"],
}
...
const VLorem = {
    name: "v-lorem",
    render(h) {
        const value = loremIpsum(this.value)
        return h("p", { class: "v-lorem" }, value)
    },
    props: ["value"],
}
...
const VPre = {
    ...
    render(h) {
        ...
        return h("pre", { class: "v-pre" }, value)
    },
    props: ["value"],
}
...
const VSkeleton = {
    ...
    render(h) {
        return renderWrapper(this, VPre, this.json)
        return vueRenderer(this, this.json)
    },
    async mounted() {
        await sleep(500)
        this.json = helpers.getSkeleton(this.target, Object)
    },
    props: ["target"],
}
...
const VImg = {
    ...
    `,
    props: ["value", "centered"],
    computed: {
        src() {
            const imglib = {
                sunset:
                    "/mnt/chromeos/MyFiles/Downloads/jordan-wozniak-xP_AGmeEa6s-unsplash.jpg",
            }
            return imglib[this.value]
        },
        containerStyle() {
            const size = this.size
                ? addUnit(this.size, "pt")
                : "50pt"
            return {
                width: size,
                height: size,
                textAlign: this.center ? "center" : "left",
...
const VIcon = {
    name: "v-icon",
    render(h) {
        const staticClass = `mdi mdi-${this.value}`
        return h("i", { staticClass })
    },
    props: ["value"],
}
...
const VInlineCode = {
    ...
    props: ["value"],
    render(h) {
        return h("code", { class: "v-code" }, this.value)
    },
}

const VTable = {
    name: "v-table",
    props: ["columns", "rows", "data"],
    render(h) {
        const children = this.getChildren()
        return h("table", { class: "v-table" }, children)
    },
    methods: {
        getType() {
...
const VInput = {
    ...
    computed: {
        id() {
            return this.label ? camelCase(this.label) : null
        },
        computedClass() {
            return this.theme
        },
...
const VInput2 = {
    ...
    render(h) {
        ...
        return h("input", {
            attrs: {
                type: "text",
                autofocus: this.focus ? 'autofocus' : 'null',
            },
            domProps: {
                value: this.value,
            },
