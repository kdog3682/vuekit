...
function renderWrapper(state, comp, value) {
    const staticClass = vueName(state)
    return isObject(comp) ? state.$createElement(comp, {staticClass, props: {value}}) : state.$createElement(comp, {staticClass}, value)
}

function vueName(state, ...args) {
    const name = isString(state)
        ? state
        : state.constructor.name == "VueRenderer"
