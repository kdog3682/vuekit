...
import {importLoader, cssLoader} from "/home/kdog3682/2024-javascript/js-toolkit/browser.js"
...
function build(text, mode) {
        ...
        const rootKey = frontMatter.main || Object.keys(lib)[0]
        ...
        const keys = findDependencies(rootKey, lib, finder)

        const [baseComponents, missing] = partitionByValues(
            keys, (key) => lib[key], (x) => x
        )
        const mapper = (c) => {
            if (mode == 'debug') {
...
function createApp(component, o = {}) {
    ...
    o.components.forEach(registerComponent)
