...
function build(text, mode) {
    ...
    if (mode == "file") {
        ...
        const exports = exportString(names)
        const text = join(components)
        return exports + "\n" + text
    } else {
        console.browser({ keys })
        console.browser({ missing })
    }
    return components
}
function registerComponent(v) {
    ...
    if (window.hasOwnProperty(name)) {
        return alert("the window already has the name: $1", name)
    }
...
function createApp(component, o = {}) {
    assignFresh(o, {
        props: {},
        components: []
    })
    ...
    const defaultApp = {
        ...
        render(h) {
            // return h('div', 'hi')
            // console.log(component)
            return h(component, { props: o.props })
        }
    }
    const components = isModule(o.components)
        ? Object.values(o.components)
        : o.components
    components.forEach(registerComponent)
    return new Vue(defaultApp)
}
...
const VInvivo = {
    ...
    props: ["value"],
    mounted() {
        console.log("hello from VInvivo")
        console.log(document.body.innerHTML)
    },
    render(h) {
        const components = build(this.value)
        components.forEach(registerComponent)
...
function registerPartialComponent(c) {
    return c
}

function partialTransform(x) {
    if (isHTML(x.template)) {
        return x
    }
    const html = vuetify(x.template)
    x.template = html
    return x
}
function create(value, components) {
    const errorHandler = (e, vm, loc) => {
        logger.error(e, vm.$options.template, loc)
    }

    flattenModule(components).map(partialTransform).forEach(registerComponent)
    Vue.config.errorHandler = errorHandler
    const app = createApp(VInvivo, { props: { value } })
...
function postfix(c) {
    console.log(c)
    // postfix on a component
    // brings in WindowControl if contextHandle is detected
    // process of bringing in ... is altering beforeCreate
    // conforms the template to standard html
    const key = 'mounted'
    if (c.contextHandle || c.firestoreConfig || c.windowStorage || c.localStorageConfig) {
        if (c[key]) {
            const prev = c[key]
            c[key] = function () {
                fn.call(this)
        ...
        function fn() {
            if (c.localStorageConfig) {
                this.localStorage = new LocalStorage(c.localStorageConfig)
            }

            if (c.windowStorage) {
                this.windowStorage = new WindowStorage(c.windowStorage)
            }

            if (c.contextHandle) {
                this.vuewin = new WindowControl(this)
            }
            if (c.firestoreConfig) {
                this.firestore = new FireStore(c.firestoreConfig)
            }
        }
    }

    if (c.template && !isHTML(c.template)) {
        c.template = vuetify(c.template, { mode: "template" })
        console.log("vuetification complete: ", c.template)
    }
    return c
}
