...
/* prettier-ignore */ import {prependOrReplace, rpx} from "/home/kdog3682/2023/node-utils.js"
import * as helpers from "./helpers.js"

const file = "/home/kdog3682/2024-javascript/my-daily-tracker-website/index.js"

// rpx(file, createRealComponent)
function createRealComponent(s, inpath) {
    const lib = codeLibrary(s, Object)
    const parts = inpath.replace(/\/$/, '').split('/')
    const dirName = parts[parts.length - 2].replace(/-?website$/, '')
    const pascal = pascalCase(dirName)
    const name = dashCase(pascal)
    if (lib.name == null) {
        lib.name = createVariable('name', name)
    }

    if (lib.data || lib.props) {
        panic('we do not proceed if data or props exists. it implies the file has already been generated')
    }

    Object.assign(lib, createPropsAndData(lib, s))

    const mapper = (key) => {
        return lib.hasOwnProperty(key) && key
        let value = lib[key]
        return value
    }
    const keys = mapFilter(variables.vueComponentOptionKeys, mapper)
    const top = sortObject(lib, variables.vueComponentOptionKeys)
    // console.log(top); throw "top"
    const obj = createVariable(pascal, brackify(keys, Array))
    const exportStatement = exportString(pascal)
    // console.log(obj)
    // console.log(exportStatement)
    const payload = join(top, obj, exportStatement)
    return payload
    console.log(payload)
    ...
    // addExports()
    // rpw(file, runner)
    // too much implicit becomes too hard to handle is the reality
    // because it is all linked together
}
function createPropsAndData(lib, text) {
    const r = /^    (\w+)(?:\(|:).*?{/gm
    const computed = findall(r, lib.computed)
    const [dataRaw, props, numerics] = helpers.getDataAndPropKeys(text)
    ...
    const data = helpers.assignFallbackValues(dataRaw)
    Object.assign(data, baseData)
    props.forEach((prop, i) => {
        if (variables.vueImplicitDataKeyRef.hasOwnProperty(prop)) {
            data[prop] = variables.vueImplicitDataKeyRef[prop]
        }
    })
    ...
    const ignore = Object.keys(data).concat(computed).concat(alwaysIgnore)
    const dataFn = buildFunction({ name: "data", body: data })
    const filteredProps = unique(props, ignore)
    const dataStr = createVariable('data', dataFn)
    const propStr = createVariable('props', filteredProps)
