...
function combineData(a, b) {
    ...
    return function combinedData(s) {
        return data
    }
}

function uniqueObject(a, b) {
    return Object.assign(a, b)
}
...
const mergeStrategy = {
    ...
    data: combineData,
    computed: combineObjects,
    watch: combineObjects,
    methods: combineObjects,
    mounted: combineFunctions,
    created: combineFunctions,
}
...
function merge(a, b) {
    ...
    for (const [k, v] of entries(b)) {
        const ref = mergeStrategy[k]
        if (ref) {
            const t = type(b)
            const merger = isObject(ref) ? must(ref, t) : ref
            a[k] = merger(a[k], v)
        } else {
            a[k] = v
        }
